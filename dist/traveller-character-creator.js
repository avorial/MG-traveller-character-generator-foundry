...
function b(l, e, s = 0, t = null, i = !1) {
  ...
  if (r)
    return s === 0 ? `Already has ${D(e, t)} ${r.level}` : i ? s > r.level ? (r.level = Math.min(s, 4), M(l.skills), `Increased ${D(e, t)} to ${r.level}`) : `${D(e, t)} unchanged (already ${r.level})` : (r.level = Math.min(r.level + s, 4), M(l.skills), `Increased ${D(e, t)} to ${r.level}`);
  const n = Math.max(0, s);
  return l.skills.push({ name: e, level: n, speciality: t }), t && n >= 1 && !l.skills.some((a) => a.name === e && !a.speciality) && l.skills.push({ name: e, level: 0, speciality: null }), M(l.skills), `Gained ${D(e, t)} ${n}`;
}
function D(l, e) {
  return `${l}${e ? ` (${e})` : ""}`;
}
...
class G {
  ...
  rollInitialCharacteristics(e, s = !1) {
    const t = h(e), i = {}, r = /* @__PURE__ */ new Set();
    if (s) {
  ...
  applySpecies(e, s) {
    ...
    if (t.hiver_species) {
      const o = this.roller.roll2D(), c = ((r = t.hiver_nest_table) == null ? void 0 : r[String(Math.max(2, Math.min(12, o.total)))]) ?? "generalist";
      i.hiver_nest_type = c;
      const u = (a = (n = t.hiver_nest_benefits) == null ? void 0 : n[c]) == null ? void 0 : a.background;
      if (u)
        for (const d of String(u).split(",")) this.applySkillOrStat(i, d.trim(), 0);
      i.notes.push(`Hiver nest type: ${c}.`);
    }
  ...
  rollDroyneCaste(e, s) {
    ...
    const i = h(e), r = s ? null : this.roller.d6(), n = s ?? ((o = t.droyne_caste_table) == null ? void 0 : o[String(r)]) ?? null;
    if (!n || !((c = t.droyne_caste_mods) != null && c[n])) throw new Error(`Unknown Droyne caste: ${s ?? r}`);
    i.droyne_caste_mods_applied || (this.applyStatBlock(i, t.droyne_casting_bonus ?? {}), this.applyStatBlock(i, t.droyne_caste_mods[n] ?? {}), i.droyne_caste_mods_applied = !0), i.droyne_caste = n, i.droyne_caste_number = r ?? Number(((u = Object.entries(t.droyne_caste_table ?? {}).find(([, d]) => d === n)) == null ? void 0 : u[0]) ?? 0);
    ...
    ], a >= 4 ? b(i, "Flight", 0, null, !0) : i.pending_life_event_choice = { kind: "droyne_vestigial_wing_skill", options: ["Drive", "Flyer", "Recon", "Survival"], level: 0, prompt: "Choose a replacement for Flight 0." }, i.notes.push(`Droyne caste: ${n}.`), { caste: n, casteRoll: r, wingRoll: a, character: i };
  }
  ...
  applyBackgroundSkills(e, s) {
    const t = h(e), i = Math.max(0, 3 + N(t.characteristics.EDU));
  ...
  rollAslanClan(e) {
    var a;
    const s = h(e), t = this.rules.species(s.species_id) ?? {}, i = ((a = this.rules.table("aslan_background").clan) == null ? void 0 : a.results) ?? {}, r = t.clan_determination === "fixed" ? null : this.roller.rollD(6), n = r ? i[String(r.total)] : { label: t.fixed_clan_name ?? "Tokouea'we", dm_ancestral_deeds: Number(t.fixed_clan_dm ?? 0) };
  ...
  rollAslanAncestry(e) {
    var u, d, _, m, g;
    const s = h(e), t = this.rules.table("aslan_background"), i = Number(((u = s.aslan_setup_status) == null ? void 0 : u.clan_dm_ancestral_deeds) ?? 0), r = this.roller.rollD(6), n = String(Math.max(1, Math.min(7, r.total + i))), a = ((_ = (d = t.ancestral_deeds) == null ? void 0 : d.results) == null ? void 0 : _[n]) ?? {};
    let o = Number(a.territory ?? 0);
    const c = [];
    for (let v = 0; v < 2; v++) {
      const k = this.roller.roll2D(), f = ((g = (m = t.past_deeds) == null ? void 0 : m.results) == null ? void 0 : g[String(Math.max(2, Math.min(12, k.total)))]) ?? {};
      c.push({ roll: k, result: f }), f.territory === "lose_all" ? o = 0 : o = Math.max(0, o + Number(f.territory ?? 0)), this.applyAslanPastDeedBonus(s, f);
    }
    return y(s, "TER", o), s.aslan_setup_status = { ...s.aslan_setup_status ?? {}, phase: "family", ancestral_territory: o }, s.notes.push(`Aslan ancestry territory: ${o}.`), { ancestralRoll: r, ancestral: a, past: c, territory: o, character: s };
  }
  rollAslanFamily(e) {
    var c;
    const s = h(e), t = ((c = this.rules.table("aslan_background").family_inheritance) == null ? void 0 : c.results) ?? {}, i = this.roller.roll2D(), r = t[String(Math.max(2, Math.min(12, i.total)))] ?? {}, n = s.gender === "female" ? "female" : "male", a = r[`label_${n}`] ?? "Family Member", o = !!r.inherits_territory;
  ...
  rollAslanRite(e) {
    var o, c;
    const s = h(e), t = this.roller.roll2D(), i = s.gender === "female" ? "female" : "male";
    let r = t.total;
    i === "male" ? r += T.filter((u) => p(s, u) > t.total).length : r += ["INT", "EDU", "SOC"].filter((u) => p(s, u) > t.total).length * 2;
  ...
  qualifyForPreCareer(e, s, t = {}) {
    var g, v, k;
    const i = (g = this.rules.table("education").tracks) == null ? void 0 : g[s];
    if (!i) throw new Error(`Unknown pre-career track: ${s}`);
    const r = h(e), n = t.service ? (v = i.services) == null ? void 0 : v[t.service] : null, a = t.curriculum ? (k = i.curricula) == null ? void 0 : k[t.curriculum] : null, o = (n == null ? void 0 : n.qualification) ?? i.qualification ?? {}, c = this.checkDm(r, o), u = o.automatic ? null : this.roller.roll2D(c), d = o.automatic || !!(u && u.total >= Number(o.target ?? 0));
    if (!d)
      return r.phase = "career", r.notes.push(`Failed ${i.name ?? s} qualification${u ? ` (${u.total})` : ""}.`), { track: i, roll: u, qualified: d, character: r };
    ...
    for (let f = 0; f < Number(i.enrollment_service_skill_random ?? 0); f++) {
      const E = this.rollOnExternalSkillTable(r, (n == null ? void 0 : n.career_id) ?? "merchant", "service_skills");
      E && m.push(E);
    }
    ...
    return r.pre_career_status = {
      ...
      skill_pool: _,
      enrollment_skills: m
    }, r.phase = "pre_career", r.notes.push(`Qualified for ${i.name ?? s}.`), { track: i, roll: u, qualified: d, character: r };
  }
  graduatePreCareer(e, s = []) {
    ...
    const n = h(e), a = r.graduation ?? {};
    if (t.forced_graduation_failure)
      return n.pre_career_status = { ...t, graduated: !1, honours: !1, graduation_roll: null, outcome_note: ((g = a.on_failure) == null ? void 0 : g.note) ?? "Failed to graduate." }, n.age += Number(r.age_cost ?? 0), n.pre_career_terms += Number(r.age_cost ?? 0) > 0 ? 1 : 0, n.phase = "career", n.notes.push(`Failed to graduate from ${r.name ?? i} due to pre-career event.`), { track: r, roll: null, graduated: !1, honours: !1, character: n };
    ...
    return d && this.applyPreCareerOutcome(n, r, _, s), n.age = Math.max(n.age + Number(r.age_cost ?? 0), this.rollAgeOverride(_.age_override) ?? 0), n.pre_career_terms += Number(r.age_cost ?? 0) > 0 ? 1 : 0, n.pre_career_status = { ...t, graduated: d, honours: u, graduation_roll: c.total, outcome_note: _.note ?? null }, n.phase = "career", n.notes.push(`${d ? u ? "Graduated with honours from" : "Graduated from" : "Failed to graduate from"} ${r.name ?? i}.`), { track: r, roll: c, graduated: d, honours: u, character: n };
  }
  preCareerEventRoll(e, s = !1) {
    const t = h(e), i = this.rules.table("education"), r = s ? i.aslan_pre_career_events : i.pre_career_events, n = this.roller.roll2D(), a = String(Math.max(2, Math.min(12, n.total))), o = String((r == null ? void 0 : r[a]) ?? "No event.");
  ...
  qualifyForCareer(e, s) {
    ...
    const i = h(e), r = this.careerBlocked(i, t);
    if (r)
      return i.notes.push(`Cannot qualify for ${t.name ?? s}: ${r}.`), { career: t, qualified: !1, blockedReason: r, character: i };
    const n = i.pending_transfer_career_id === "any" || i.pending_transfer_career_id === s, a = n || i.auto_entry_career_id === s || i.auto_qualify_career_ids.includes(s), o = this.checkDm(i, t.qualification ?? {}) + i.dm_next_qualification + Number(i.permanent_qualification_dm_by_career[s] ?? 0) - i.failed_qualifications_this_term, c = a || (d = t.qualification) != null && d.automatic ? null : this.roller.roll2D(o), u = a || ((_ = t.qualification) == null ? void 0 : _.automatic) || !!(c && c.total >= Number(((m = t.qualification) == null ? void 0 : m.target) ?? 0));
    return i.dm_next_qualification = 0, u ? (i.failed_qualifications_this_term = 0, n && (i.pending_transfer_career_id = null), i.auto_qualify_career_ids = i.auto_qualify_career_ids.filter((g) => g !== s), i.notes.push(`Qualified for ${t.name ?? s}.`)) : (i.failed_qualifications_this_term += 1, i.notes.push(`Failed qualification for ${t.name ?? s}${c ? ` (${c.total})` : ""}.`)), { career: t, roll: c, qualified: u, character: i };
  }
  startTerm(e, s, t) {
    var g, v;
    ...
    const o = h(e), c = o.term_history.filter((k) => k.career_id === s).length, u = !!i.all_commissioned || o.starts_commissioned_career_id === s || !!o.completed_careers.find((k) => k.career_id === s && k.commissioned), d = o.pending_transfer_career_id === s || o.pending_transfer_career_id === "any" ? o.pending_transfer_rank : null, _ = d != null ? Number(d) : u ? Number(o.starts_commissioned_rank ?? 1) : 0, m = {
      ...
      advanced: null,
      mishap: null,
      basic_training: c === 0 && !i.hiver_no_basic_training,
      benefit_forfeited: !1,
  ...
  rollOnSkillTable(e, s) {
    const t = h(e), i = this.requireCurrentTerm(t), r = this.rules.career(i.career_id), n = this.rollOnCareerSkillTable(t, r, s);
    return n.note && i.skills_gained.push(n.note), { career: r, tableId: s, roll: n.roll, result: n.entry, character: t };
  }
  survivalRoll(e) {
    const s = h(e), t = this.requireCurrentTerm(s), i = this.rules.career(t.career_id);
    if (i.no_survival)
    ...
    const r = this.assignmentData(i, t.assignment_id), n = i.survival ?? r.survival ?? {}, a = this.checkDm(s, n) + s.dm_next_survival, o = this.roller.roll2D(a), c = o.natural !== 2 && o.total >= Number(n.target ?? 0);
    return t.survived = c, t.survival_roll_total = o.total, s.dm_next_survival = 0, c || t.events.push("Failed survival roll; roll on the Mishap table."), s.notes.push(`${c ? "Passed" : "Failed"} survival in ${i.name ?? t.career_id}.`), { career: i, roll: o, survived: c, character: s };
  }
  eventRoll(e) {
    var o;
    const s = h(e), t = this.requireCurrentTerm(s), i = this.rules.career(t.career_id), r = this.roller.roll2D(s.dm_next_events), n = String(((o = i.events) == null ? void 0 : o[String(Math.max(2, Math.min(12, r.total)))]) ?? "No event.");
  ...
  lifeEventRoll(e, s = !1) {
    var d;
    const t = h(e), i = s ? (d = this.rules.table("aslan_life_events").aslan_life_events) == null ? void 0 : d.results : this.rules.table("life_events").entries, r = this.roller.roll2D(), n = String(Math.max(2, Math.min(12, r.total))), a = i == null ? void 0 : i[n], o = typeof a == "string" ? a.split(":")[0] : (a == null ? void 0 : a.title) ?? "Life Event", c = typeof a == "string" ? a : (a == null ? void 0 : a.text) ?? "Life Event.";
    let u = null;
    if (!s && (a != null && a.sub_table)) {
      const _ = this.roller.rollD(6);
      return u = String(a.sub_table[String(_.total)] ?? ""), this.applyLifeEventEffects(t, o, `${c} ${u}`, s), t.notes.push(`Life event: ${o}; ${u}`), { roll: r, event: { title: o, text: c }, subEvent: u, character: t };
    }
    return this.applyLifeEventEffects(t, o, c, s), t.notes.push(`Life event: ${o}.`), { roll: r, event: { title: o, text: c }, character: t };
  }
  resolveLifeEventChoice(e, s) {
    const t = h(e), i = t.pending_life_event_choice;
    ...
    if (r === "relationship_end" || r === "betrayal") {
      ...
      const n = Number(i.level ?? 0), [a, o, c] = $(/\d+$/.test(s) ? s : `${s} ${n}`);
      String(i.excluded ?? "").includes(a) || b(t, a, c, o, !0);
  ...
  testPsionics(e) {
    ...
    const t = h(e), i = this.rules.table("psionics"), r = Number(((d = i.potential_test) == null ? void 0 : d.target) ?? 9), n = -t.total_terms, a = this.roller.roll2D(n);
    ...
    const o = this.roller.roll2D(), c = i.psi_strength_formula ?? {}, u = Math.max(Number(c.min ?? 0), Math.min(Number(c.max ?? 15), o.total - t.total_terms));
  ...
  trainPsionicTalent(e, s) {
    ...
    const i = (u = this.rules.table("psionics").talents) == null ? void 0 : u[s];
    if (!i) throw new Error(`Unknown psionic talent: ${s}`);
    const r = h(e), n = (d = r.pre_career_status) != null && d.pending_psionic_training ? 0 : Number(i.cost_cr ?? 2e5), a = Math.min(r.credits, n);
    r.credits -= a, r.medical_debt += n - a;
    const o = this.roller.roll2D(N(r.psi)), c = o.total >= Number(i.test_target ?? 8);
    return c && (b(r, String(i.skill ?? i.name), 0, null, !0), r.psi_trained_talents.push(s)), r.notes.push(`Psionic training ${i.name}: ${c ? "passed" : "failed"}.`), { talentId: s, talent: i, roll: o, succeeded: c, cost: n, debtIncurred: n - a, character: r };
  }
  mishapRoll(e) {
    var o;
    const s = h(e), t = this.requireCurrentTerm(s), i = this.rules.career(t.career_id), r = this.roller.rollD(6), n = String(((o = i.mishaps) == null ? void 0 : o[String(Math.max(1, Math.min(6, r.total)))]) ?? "Mishap.");
    t.mishap = n;
  ...
  advancementRoll(e) {
    ...
    const r = this.assignmentData(i, t.assignment_id), n = i.advancement ?? r.advancement ?? {}, a = this.checkDm(s, n) + s.dm_next_advancement + s.dm_permanent_advancement + Number(s.permanent_advancement_dm_by_career[t.career_id] ?? 0), o = this.roller.roll2D(a), c = o.total >= Number(n.target ?? 0);
    return t.advanced = c, t.advancement_roll_total = o.total, s.dm_next_advancement = 0, c && (t.rank = Math.min(6, t.rank + 1), t.rank_title = this.rankTitle(i, t.commissioned, t.rank), this.applyRankBonus(s, i, t)), s.notes.push(`${c ? "Advanced" : "Did not advance"} in ${i.name ?? t.career_id}.`), { career: i, roll: o, advanced: c, character: s };
  }
  commissionRoll(e) {
    const s = h(e), t = this.requireCurrentTerm(s), i = this.rules.career(t.career_id), r = i.commission;
  ...
  endTerm(e, s = !1, t = "voluntary") {
    ...
    if (i.current_term = null, i.failed_qualifications_this_term = 0, s || i.force_career_end || r.survived === !1) {
      const c = i.term_history.filter((d) => d.career_id === r.career_id).length, u = n.mustering_out === null ? 0 : this.benefitRollsEarned(c * Number(n.mustering_out_rolls_per_term ?? 1), r.rank, r.benefit_forfeited);
      i.pending_benefit_rolls += u, i.completed_careers.push({
  ...
  musterOutRoll(e, s, t = "benefit") {
    var B;
    const i = h(e), r = s ? [...i.completed_careers].reverse().find((w) => w.career_id === s) : i.completed_careers[i.completed_careers.length - 1];
    ...
    const a = r.final_rank >= 5 ? 1 : 0, o = t === "cash" && i.skills.some((w) => w.name.toLowerCase() === "gambler") ? 1 : 0, c = n.mustering_out_dm_characteristic ? N(p(i, n.mustering_out_dm_characteristic)) : 0, u = i.dm_next_benefit + a + o + c, d = n.hiver_career ? this.roller.roll2D(u) : this.roller.rollD(6), _ = Object.keys(n.mustering_out ?? {}).filter((w) => /^\d+$/.test(w)).map(Number), m = Math.min(..._, n.hiver_career ? 2 : 1), g = Math.max(..._, 7), v = Math.max(m, Math.min(g, d.total + (n.hiver_career ? 0 : u))), k = ((B = n.mustering_out) == null ? void 0 : B[String(v)]) ?? {}, f = t === "cash" && i.cash_rolls_used < 3 && k.cash != null ? "cash" : "benefit", E = k[f];
    if (f === "cash") {
      const w = Number(E ?? 0);
      if (w < 0)
      ...
      this.applyMusterBenefit(i, String(E ?? "Benefit"));
    return i.pending_benefit_rolls -= 1, r.benefit_rolls_used += 1, i.dm_next_benefit = 0, i.pending_benefit_rolls <= 0 && (i.phase = "skill_package"), i.notes.push(`Mustering out ${f}: ${E}.`), { career: n, roll: d, tableRoll: v, column: f, result: E, character: i };
  }
  applyInjury(e, s) {
    var o;
    const t = h(e), i = s ? { dice: [], natural: s, total: s, dm: 0 } : this.roller.rollD(6), n = ((o = this.rules.table("injury").entries) == null ? void 0 : o[String(Math.max(1, Math.min(6, i.total)))]) ?? {}, a = this.injuryPending(n, i.total);
  ...
  resolveInjuryChoice(e, s) {
    const t = h(e), i = t.pending_injury_choice;
    ...
    const n = Number(i.damage_to_chosen ?? 0), a = Number(i.auto_reduce_others ?? 0), o = ["STR", "DEX", "END"].filter((m) => m !== s), c = Math.min(p(t, s), n), u = o.map((m) => ({ stat: m, loss: Math.min(p(t, m), a) })).filter((m) => m.loss > 0), d = c + u.reduce((m, g) => m + g.loss, 0), _ = d * 5e3;
  ...
  resolveInjuryPayment(e, s) {
    const t = h(e), i = t.pending_injury_treatment_choice;
  ...
  applyChosenSkills(e, s, t, i, r) {
    ...
    for (const c of a.slice(0, Math.max(0, i))) {
      const u = t.find((g) => g.toLowerCase() === c.toLowerCase()) ?? c, [d, _, m] = $(/\d+$/.test(u.trim()) ? u : `${u} ${r}`);
      o.push(b(e, d, m, _, !0));
    }
    return o;
  }
  ...
  hiverAdvancementRoll(e, s, t) {
    var _, m, g, v;
    const i = this.rules.species(e.species_id) ?? this.rules.species("hiver") ?? {}, r = s.hiver_advancement_table ?? i.hiver_advancement_table ?? {}, n = N(p(e, "SOC")) + e.dm_next_advancement + e.dm_permanent_advancement, a = this.roller.roll2D(n), o = Number(r.senior_min ?? 10), c = Number(r.manipulator_min ?? 15), u = t.rank;
    let d = u;
    if (a.total >= c && u < 2 ? d = 2 : a.total >= o && u < 1 && (d = 1), t.advanced = d > u, t.advancement_roll_total = a.total, e.dm_next_advancement = 0, d > u) {
      ...
      if (d === 2 && !e.hiver_manipulator_bonus_awarded) {
        ...
        const k = s.hiver_manipulator_bonus ?? ((v = (g = i.hiver_nest_benefits) == null ? void 0 : g[e.hiver_nest_type ?? "generalist"]) == null ? void 0 : v.manipulator_bonus);
  ...
  rollOnCareerSkillTable(e, s, t) {
    var c;
    const i = (c = s.skill_tables) == null ? void 0 : c[t];
  ...
  applySkillOrStat(e, s, t) {
    ...
    if (!n) return null;
    const [a, o, c] = $(/\d+$/.test(n) ? n : `${n} ${t}`), u = typeof o == "string" && o.toLowerCase() === "any" ? null : o;
    return b(e, K(a), c, u, !0);
  }
  ...
  applyCareerTextEffects(e, s, t, i) {
    ...
    if (n) {
      const u = i ? "pending_career_mishap_choice" : "pending_career_event_choice";
      e[u] = {
        kind: "stat_choice",
  ...
  resolveCareerChoice(e, s, t) {
    const i = h(e), r = s === "event" ? "pending_career_event_choice" : "pending_career_mishap_choice", n = i[r];
    ...
    if (a === "skill_choice" || a === "free_skill_choice") {
      ...
      const o = Number(n.level ?? 1), [c, u, d] = $(/\d+$/.test(t) ? t : `${t} ${o}`);
      b(i, c, d, u, !0);
      ...
      const o = this.roller.roll2D(this.skillDm(i, t)), c = o.total >= Number(n.target ?? 8);
      i.notes.push(`${t} check ${c ? "succeeded" : "failed"} (${o.total}).`);
      const u = Array.isArray(n.successSkillOptions) ? n.successSkillOptions : [];
      if (c && u.length)
        return i.pending_career_event_choice = { kind: "skill_choice", options: u, level: 1, prompt: n.prompt }, { roll: o, succeeded: c, character: i };
      if (!c && /Mishap/i.test(String(n.prompt ?? "")) && i.current_term) {
  ...
  skillDm(e, s) {
    const [t, i] = q(s), r = t.toLowerCase(), n = (i == null ? void 0 : i.toLowerCase()) ?? null, a = e.skills.find((c) => {
      var u;
      return c.name.toLowerCase() === r && (((u = c.speciality ?? null) == null ? void 0 : u.toLowerCase()) ?? null) === n;
    }), o = e.skills.find((c) => c.name.toLowerCase() === r && !c.speciality);
  ...
  applySingleMusterBenefit(e, s) {
    ...
    if (i) {
      const u = i[1].toUpperCase() === "D3" ? this.roller.d3() : this.roller.d6();
  ...
  applyAgingIfNeeded(e) {
    ...
    const i = this.roller.roll2D(-e.total_terms), r = this.rules.table("aging"), n = this.agingEntry(r, i.total), a = this.applyAgingEffects(e, n.effects ?? []), o = a.some((c) => p(e, c.stat) <= 0);
    if (o) {
  ...
  applyAgingEffects(e, s) {
    ...
    for (const n of s) {
      const a = n.type === "reduce_mental" ? r : i, o = Math.min(Number(n.count ?? 1), a.length), c = Number(n.amount ?? 0);
...
function W(l) {
  ...
  if (!e) return null;
  const s = Number(e[2] ?? e[4] ?? 8), t = [e[1], e[3]].filter(Boolean).map((r) => String(r).trim()), i = P(l);
...
function ne(l, e = {}) {
  const s = e.entryYear ?? 1105, t = ae(l), i = Object.fromEntries(z.map((o) => {
    ...
    return [o, { value: c, current: c, show: de(o, c), default: !1 }];
...
`))
  ...
  for (const r of l.kkree_family_members)
    e.push(A(String(r.name ?? r.role ?? "K'kree Family Member"), "associate", {
...
function ce(l) {
  return l.term_history.map((e, s) => {
    const t = C(e.career_id.replaceAll("_", " ")), i = C(e.assignment_id.replaceAll("_", " ")), r = `${t}${i ? `: ${i}` : ""}${e.rank_title ? ` (${e.rank_title})` : ""}`, n = [r, ...e.events.map((a) => `* ${a}`)].join(`
`);
    return A(`Term ${s + 1}: ${r}`, "term", {
