...
function b(l, e, s = 0, t = null, r = !1) {
  ...
  if (i)
    return s === 0 ? `Already has ${$(e, t)} ${i.level}` : r ? s > i.level ? (i.level = Math.min(s, 4), M(l.skills), `Increased ${$(e, t)} to ${i.level}`) : `${$(e, t)} unchanged (already ${i.level})` : (i.level = Math.min(i.level + s, 4), M(l.skills), `Increased ${$(e, t)} to ${i.level}`);
  const n = Math.max(0, s);
  return l.skills.push({ name: e, level: n, speciality: t }), t && n >= 1 && !l.skills.some((a) => a.name === e && !a.speciality) && l.skills.push({ name: e, level: 0, speciality: null }), M(l.skills), `Gained ${$(e, t)} ${n}`;
}
function $(l, e) {
  return `${l}${e ? ` (${e})` : ""}`;
}
...
class H {
  ...
  applySpecies(e, s) {
    ...
    if (t.extra_characteristics_required)
      for (const o of t.extra_characteristics_required)
        f(r, o) || g(r, o, this.roller.roll2D().total);
    if (t.hiver_species) {
  ...
  applyBackgroundPackage(e, s, t = {}) {
    ...
    for (const [a, o] of Object.entries(i.characteristic_modifiers ?? i.stat_mods ?? {}))
      g(n, a, f(n, a) + Number(o));
    for (const a of i.skills ?? []) {
      const o = typeof a == "string" ? a : `${a.name}${a.speciality ? ` (${a.speciality})` : ""}`, u = t[o] ?? a;
      if (typeof u == "string") {
        const [c, d, _] = C(u);
        b(n, c, _ === 1 && !/\d+$/.test(u.trim()) ? 0 : _, d);
      } else
        b(n, u.name, Number(u.level ?? 0), u.speciality ?? null);
    }
  ...
  beginAslanSetup(e) {
    ...
    return s.phase = "aslan_setup", s.aslan_setup_status = {
      ...
      rite_score: 0
    }, f(s, "TER") || g(s, "TER", 0), s.notes.push("Aslan background setup started."), { phase: "gender", character: s };
  }
  ...
  rollAslanAncestry(e) {
    var c, d, _, p, m;
    const s = h(e), t = this.rules.table("aslan_background"), r = Number(((c = s.aslan_setup_status) == null ? void 0 : c.clan_dm_ancestral_deeds) ?? 0), i = this.roller.rollD(6), n = String(Math.max(1, Math.min(7, i.total + r))), a = ((_ = (d = t.ancestral_deeds) == null ? void 0 : d.results) == null ? void 0 : _[n]) ?? {};
    let o = Number(a.territory ?? 0);
    const u = [];
    for (let v = 0; v < 2; v++) {
      const k = this.roller.roll2D(), y = ((m = (p = t.past_deeds) == null ? void 0 : p.results) == null ? void 0 : m[String(Math.max(2, Math.min(12, k.total)))]) ?? {};
  ...
  rollAslanFamily(e) {
    var u;
    const s = h(e), t = ((u = this.rules.table("aslan_background").family_inheritance) == null ? void 0 : u.results) ?? {}, r = this.roller.roll2D(), i = t[String(Math.max(2, Math.min(12, r.total)))] ?? {}, n = s.gender === "female" ? "female" : "male", a = i[`label_${n}`] ?? "Family Member", o = !!i.inherits_territory;
  ...
  rollAslanRite(e) {
    ...
    let i = t.total;
    r === "male" ? i += N.filter((c) => f(s, c) > t.total).length : i += ["INT", "EDU", "SOC"].filter((c) => f(s, c) > t.total).length * 2;
  ...
  qualifyForPreCareer(e, s, t = {}) {
    var m, v, k;
    const r = (m = this.rules.table("education").tracks) == null ? void 0 : m[s];
    if (!r) throw new Error(`Unknown pre-career track: ${s}`);
    const i = h(e), n = t.service ? (v = r.services) == null ? void 0 : v[t.service] : null, a = t.curriculum ? (k = r.curricula) == null ? void 0 : k[t.curriculum] : null, o = (n == null ? void 0 : n.qualification) ?? r.qualification ?? {}, u = this.checkDm(i, o), c = o.automatic ? null : this.roller.roll2D(u), d = o.automatic || !!(c && c.total >= Number(o.target ?? 0));
    if (!d)
  ...
  qualifyForCareer(e, s) {
    ...
    return r.dm_next_qualification = 0, c ? (r.failed_qualifications_this_term = 0, n && (r.pending_transfer_career_id = null), r.auto_qualify_career_ids = r.auto_qualify_career_ids.filter((m) => m !== s), r.notes.push(`Qualified for ${t.name ?? s}.`)) : (r.failed_qualifications_this_term += 1, r.notes.push(`Failed qualification for ${t.name ?? s}${u ? ` (${u.total})` : ""}.`)), { career: t, roll: u, qualified: c, character: r };
  }
  startTerm(e, s, t) {
    var p;
    ...
    const a = h(e), o = a.term_history.filter((m) => m.career_id === s).length, u = !!r.all_commissioned || a.starts_commissioned_career_id === s || !!a.completed_careers.find((m) => m.career_id === s && m.commissioned), c = a.pending_transfer_career_id === s || a.pending_transfer_career_id === "any" ? a.pending_transfer_rank : null, d = c != null ? Number(c) : u ? Number(a.starts_commissioned_rank ?? 1) : 0, _ = {
      career_id: s,
      assignment_id: n,
      term_number: o + 1,
    ...
    if (a.current_term = _, a.pending_transfer_career_id = null, a.pending_transfer_rank = null, _.basic_training) {
      for (const m of Object.values(((p = r.skill_tables) == null ? void 0 : p.service_skills) ?? {}).filter((v) => typeof v == "string")) {
        const v = this.applySkillOrStat(a, m, 0);
        v && _.skills_gained.push(v);
      }
      this.applyRankBonus(a, r, _);
    }
    for (const m of r.career_start_skills ?? []) {
      const v = this.applySkillOrStat(a, String(m), 0);
      v && _.skills_gained.push(v);
    }
  ...
  resolveLifeEventChoice(e, s) {
    ...
    if (i === "relationship_end" || i === "betrayal") {
      ...
      const n = Number(r.level ?? 0), [a, o, u] = C(/\d+$/.test(s) ? s : `${s} ${n}`);
  ...
  trainPsionicTalent(e, s) {
    ...
    return u && (b(i, String(r.skill ?? r.name), 0, null, !0), i.psi_trained_talents.push(s)), i.notes.push(`Psionic training ${r.name}: ${u ? "passed" : "failed"}.`), { talentId: s, talent: r, roll: o, succeeded: u, cost: n, debtIncurred: n - a, character: i };
  }
  mishapRoll(e) {
    var o;
  ...
  commissionRoll(e) {
    ...
    if (t.term_number > 1 && f(s, "SOC") < 9) throw new Error("Commission after the first term requires SOC 9+.");
  ...
  musterOutRoll(e, s, t = "benefit") {
    var B;
    const r = h(e), i = s ? [...r.completed_careers].reverse().find((E) => E.career_id === s) : r.completed_careers[r.completed_careers.length - 1];
    ...
    const a = i.final_rank >= 5 ? 1 : 0, o = t === "cash" && r.skills.some((E) => E.name.toLowerCase() === "gambler") ? 1 : 0, u = n.mustering_out_dm_characteristic ? A(f(r, n.mustering_out_dm_characteristic)) : 0, c = r.dm_next_benefit + a + o + u, d = n.hiver_career ? this.roller.roll2D(c) : this.roller.rollD(6), _ = Object.keys(n.mustering_out ?? {}).filter((E) => /^\d+$/.test(E)).map(Number), p = Math.min(..._, n.hiver_career ? 2 : 1), m = Math.max(..._, 7), v = Math.max(p, Math.min(m, d.total + (n.hiver_career ? 0 : c))), k = ((B = n.mustering_out) == null ? void 0 : B[String(v)]) ?? {}, y = t === "cash" && r.cash_rolls_used < 3 && k.cash != null ? "cash" : "benefit", w = k[y];
    if (y === "cash") {
      const E = Number(w ?? 0);
      if (E < 0)
  ...
  resolveInjuryChoice(e, s) {
    ...
    const n = Number(r.damage_to_chosen ?? 0), a = Number(r.auto_reduce_others ?? 0), o = ["STR", "DEX", "END"].filter((p) => p !== s), u = Math.min(f(t, s), n), c = o.map((p) => ({ stat: p, loss: Math.min(f(t, p), a) })).filter((p) => p.loss > 0), d = u + c.reduce((p, m) => p + m.loss, 0), _ = d * 5e3;
  ...
  resolveInjuryPayment(e, s) {
    ...
    if (s)
      ...
      g(t, i, f(t, i) - Number(r.damage_to_chosen ?? 0));
  ...
  applyStatBlock(e, s) {
    for (const [t, r] of Object.entries(s))
      (N.includes(t) || t === "PSI" || t === "CHA") && (g(e, t, f(e, t) + Number(r)), t === "PSI" && (e.psi = f(e, "PSI")));
  }
  applyPreCareerOutcome(e, s, t, r) {
    var o, u, c, d, _, p;
    this.applyStatBlock(e, t), t.EDU_penalty_dice === "D3" && g(e, "EDU", f(e, "EDU") - this.roller.d3()), t.jack_of_all_trades && b(e, "Jack-of-All-Trades", Number(t.jack_of_all_trades), null, !0), this.applySkillResults(e, t.fixed_skills ?? [], 1);
    ...
    for (const m of t.associates ?? [])
      e.associates.push({ kind: m.kind ?? "contact", description: m.description ?? `${s.name} associate` });
    const a = t.permanent ?? {};
  ...
  applyChosenSkills(e, s, t, r, i) {
    ...
    for (const u of a.slice(0, Math.max(0, r))) {
      const c = t.find((m) => m.toLowerCase() === u.toLowerCase()) ?? u, [d, _, p] = C(/\d+$/.test(c.trim()) ? c : `${c} ${i}`);
      o.push(b(e, d, p, _, !0));
    }
    return o;
  }
  ...
  careerBlocked(e, s) {
    ...
    if ((i = s.hiver_open_to) != null && i.length && e.species_id === "hiver" && !s.hiver_open_to.includes("any") && !s.hiver_open_to.includes(e.hiver_nest_type)) {
    ...
    for (const p of ((n = s.qualification) == null ? void 0 : n.modifiers) ?? []) {
      if (p.type === "soc_minimum" && Number(p.dm ?? 0) === 0 && f(e, "SOC") < Number(p.soc ?? 0)) return `requires SOC ${p.soc}+`;
      if (p.type === "soc_maximum" && Number(p.dm ?? 0) === 0 && f(e, "SOC") > Number(p.soc ?? 0)) return `requires SOC ${p.soc}-`;
    }
  ...
  hiverAdvancementRoll(e, s, t) {
    var _, p, m, v;
    const r = this.rules.species(e.species_id) ?? this.rules.species("hiver") ?? {}, i = s.hiver_advancement_table ?? r.hiver_advancement_table ?? {}, n = A(f(e, "SOC")) + e.dm_next_advancement + e.dm_permanent_advancement, a = this.roller.roll2D(n), o = Number(i.senior_min ?? 10), u = Number(i.manipulator_min ?? 15), c = t.rank;
    let d = c;
    if (a.total >= u && c < 2 ? d = 2 : a.total >= o && c < 1 && (d = 1), t.advanced = d > c, t.advancement_roll_total = a.total, e.dm_next_advancement = 0, d > c) {
      ...
      if (d === 2 && !e.hiver_manipulator_bonus_awarded) {
        ...
        const k = s.hiver_manipulator_bonus ?? ((v = (m = r.hiver_nest_benefits) == null ? void 0 : m[e.hiver_nest_type ?? "generalist"]) == null ? void 0 : v.manipulator_bonus);
  ...
  rollOnCareerSkillTable(e, s, t) {
    ...
    if (r.requires_edu && f(e, "EDU") < Number(r.requires_edu)) throw new Error(`${r.name ?? t} requires EDU ${r.requires_edu}+.`);
  ...
  applySkillOrStat(e, s, t) {
    ...
    if (!n) return null;
    const [a, o, u] = C(/\d+$/.test(n) ? n : `${n} ${t}`), c = typeof o == "string" && o.toLowerCase() === "any" ? null : o;
  ...
  resolveCareerChoice(e, s, t) {
    ...
    if (a === "skill_choice" || a === "free_skill_choice") {
      ...
      const o = Number(n.level ?? 1), [u, c, d] = C(/\d+$/.test(t) ? t : `${t} ${o}`);
      b(r, u, d, c, !0);
  ...
  applyPreCareerEventEffects(e, s, t, r) {
    if (/Carouse 1/i.test(t) && b(e, "Carouse", 1, null, !0), /Increase your SOC by \+1/i.test(t) && g(e, "SOC", f(e, "SOC") + 1), /Gain D3 Allies/i.test(t)) {
  ...
  applySingleMusterBenefit(e, s) {
    ...
    if (o) {
      ...
      c === "REP" ? e.reputation += Number(o[2]) : c === "RES" ? g(e, "SOC", f(e, "SOC") + Number(o[2])) : g(e, c, f(e, c) + Number(o[2])), c === "PSI" && (e.psi = f(e, "PSI"));
      return;
    }
  ...
  applyAgingIfNeeded(e) {
    ...
    const r = this.roller.roll2D(-e.total_terms), i = this.rules.table("aging"), n = this.agingEntry(i, r.total), a = this.applyAgingEffects(e, n.effects ?? []), o = a.some((u) => f(e, u.stat) <= 0);
    if (o) {
  ...
  applyAgingEffects(e, s) {
    ...
    for (const n of s) {
      ...
      for (const c of a.slice(0, o))
        g(e, c, f(e, c) - u), t.push({ stat: c, amount: u });
    }
    return t;
  }
  finalizeRobot(e) {
...
function Z(l) {
  ...
  if (!e) return null;
  const s = Number(e[2] ?? e[4] ?? 8), t = [e[1], e[3]].filter(Boolean).map((i) => String(i).trim()), r = j(l);
