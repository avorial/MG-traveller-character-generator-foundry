...
class H {
  ...
  rollInitialCharacteristics(e, s = !1) {
    const t = f(e), r = {}, n = /* @__PURE__ */ new Set();
    if (s) {
  ...
  applyBackgroundSkills(e, s) {
    const t = f(e), r = Math.max(0, 3 + A(t.characteristics.EDU));
  ...
  applyBackgroundPackage(e, s, t = {}) {
    ...
    for (const a of n.skills ?? []) {
      ...
      if (typeof c == "string") {
        const [u, d, _] = w(c);
        k(i, u, _ === 1 && !/\d+$/.test(c.trim()) ? 0 : _, d);
      } else
        k(i, c.name, Number(c.level ?? 0), c.speciality ?? null);
    }
  ...
  applyCareerPackage(e, s) {
    ...
    for (const a of n.skills ?? [])
      if (typeof a == "string") {
        ...
        k(i, a.name, Number(a.level ?? 0), a.speciality ?? null, !0);
    i.credits += Number(n.credits ?? 0);
    for (const a of n.equipment ?? []) i.equipment.push({ name: String(a), quantity: 1, notes: null });
  ...
  rollAslanClan(e) {
    var a;
    const s = f(e), t = this.rules.species(s.species_id) ?? {}, r = ((a = this.rules.table("aslan_background").clan) == null ? void 0 : a.results) ?? {}, n = t.clan_determination === "fixed" ? null : this.roller.rollD(6), i = n ? r[String(n.total)] : { label: t.fixed_clan_name ?? "Tokouea'we", dm_ancestral_deeds: Number(t.fixed_clan_dm ?? 0) };
  ...
  rollAslanAncestry(e) {
    var u, d, _, m, p;
    const s = f(e), t = this.rules.table("aslan_background"), r = Number(((u = s.aslan_setup_status) == null ? void 0 : u.clan_dm_ancestral_deeds) ?? 0), n = this.roller.rollD(6), i = String(Math.max(1, Math.min(7, n.total + r))), a = ((_ = (d = t.ancestral_deeds) == null ? void 0 : d.results) == null ? void 0 : _[i]) ?? {};
    ...
    for (let v = 0; v < 2; v++) {
      ...
      c.push({ roll: b, result: y }), y.territory === "lose_all" ? o = 0 : o = Math.max(0, o + Number(y.territory ?? 0)), this.applyAslanPastDeedBonus(s, y);
    }
  ...
  rollAslanFamily(e) {
    var c;
    const s = f(e), t = ((c = this.rules.table("aslan_background").family_inheritance) == null ? void 0 : c.results) ?? {}, r = this.roller.roll2D(), n = t[String(Math.max(2, Math.min(12, r.total)))] ?? {}, i = s.gender === "female" ? "female" : "male", a = n[`label_${i}`] ?? "Family Member", o = !!n.inherits_territory;
  ...
  rollAslanRite(e) {
    ...
    let n = t.total;
    r === "male" ? n += N.filter((u) => h(s, u) > t.total).length : n += ["INT", "EDU", "SOC"].filter((u) => h(s, u) > t.total).length * 2;
  ...
  qualifyForPreCareer(e, s, t = {}) {
    ...
    const n = f(e), i = t.service ? (v = r.services) == null ? void 0 : v[t.service] : null, a = t.curriculum ? (b = r.curricula) == null ? void 0 : b[t.curriculum] : null, o = (i == null ? void 0 : i.qualification) ?? r.qualification ?? {}, c = this.checkDm(n, o), u = o.automatic ? null : this.roller.roll2D(c), d = o.automatic || !!(u && u.total >= Number(o.target ?? 0));
    if (!d)
    ...
    const _ = this.preCareerSkillPool(r, i, a), m = this.applyChosenSkills(n, t.skills, _, Number(r.enrollment_skill_picks ?? 0), Number(r.enrollment_pick_level ?? 0));
  ...
  graduatePreCareer(e, s = []) {
    var m, p;
    const t = e.pre_career_status ?? {}, r = String(t.track_id ?? ""), n = (m = this.rules.table("education").tracks) == null ? void 0 : m[r];
    if (!n) throw new Error("No active pre-career track to graduate.");
    const i = f(e), a = n.graduation ?? {};
  ...
  preCareerEventRoll(e, s = !1) {
    const t = f(e), r = this.rules.table("education"), n = s ? r.aslan_pre_career_events : r.pre_career_events, i = this.roller.roll2D(), a = String(Math.max(2, Math.min(12, i.total))), o = String((n == null ? void 0 : n[a]) ?? "No event.");
  ...
  qualifyForCareer(e, s) {
    ...
    const r = f(e), n = this.careerBlocked(r, t);
    if (n)
  ...
  startTerm(e, s, t) {
    ...
    const a = f(e), o = a.term_history.filter((p) => p.career_id === s).length, c = !!r.all_commissioned || a.starts_commissioned_career_id === s || !!a.completed_careers.find((p) => p.career_id === s && p.commissioned), u = a.pending_transfer_career_id === s || a.pending_transfer_career_id === "any" ? a.pending_transfer_rank : null, d = u != null ? Number(u) : c ? Number(a.starts_commissioned_rank ?? 1) : 0, _ = {
      career_id: s,
      assignment_id: i,
      term_number: o + 1,
  ...
  rollOnSkillTable(e, s) {
    const t = f(e), r = this.requireCurrentTerm(t), n = this.rules.career(r.career_id), i = this.rollOnCareerSkillTable(t, n, s);
  ...
  eventRoll(e) {
    var o;
    const s = f(e), t = this.requireCurrentTerm(s), r = this.rules.career(t.career_id), n = this.roller.roll2D(s.dm_next_events), i = String(((o = r.events) == null ? void 0 : o[String(Math.max(2, Math.min(12, n.total)))]) ?? "No event.");
  ...
  resolveLifeEventChoice(e, s) {
    ...
    if (n === "relationship_end" || n === "betrayal") {
      ...
      const i = Number(r.level ?? 0), [a, o, c] = w(/\d+$/.test(s) ? s : `${s} ${i}`);
      String(r.excluded ?? "").includes(a) || k(t, a, c, o, !0);
  ...
  testPsionics(e) {
    ...
    const t = f(e), r = this.rules.table("psionics"), n = Number(((d = r.potential_test) == null ? void 0 : d.target) ?? 9), i = -t.total_terms, a = this.roller.roll2D(i);
    ...
    return t.psi = u, g(t, "PSI", u), t.notes.push(`Psionic potential test passed; PSI ${u}.`), { potentialRoll: a, potentialSucceeded: !0, psiRoll: o, psi: u, character: t };
  }
  trainPsionicTalent(e, s) {
    ...
    const n = f(e), i = (d = n.pre_career_status) != null && d.pending_psionic_training ? 0 : Number(r.cost_cr ?? 2e5), a = Math.min(n.credits, i);
    n.credits -= a, n.medical_debt += i - a;
    const o = this.roller.roll2D(A(n.psi)), c = o.total >= Number(r.test_target ?? 8);
    return c && (k(n, String(r.skill ?? r.name), 0, null, !0), n.psi_trained_talents.push(s)), n.notes.push(`Psionic training ${r.name}: ${c ? "passed" : "failed"}.`), { talentId: s, talent: r, roll: o, succeeded: c, cost: i, debtIncurred: i - a, character: n };
  }
  mishapRoll(e) {
    var o;
    const s = f(e), t = this.requireCurrentTerm(s), r = this.rules.career(t.career_id), n = this.roller.rollD(6), i = String(((o = r.mishaps) == null ? void 0 : o[String(Math.max(1, Math.min(6, n.total)))]) ?? "Mishap.");
    t.mishap = i;
  ...
  advancementRoll(e) {
    ...
    const n = this.assignmentData(r, t.assignment_id), i = r.advancement ?? n.advancement ?? {}, a = this.checkDm(s, i) + s.dm_next_advancement + s.dm_permanent_advancement + Number(s.permanent_advancement_dm_by_career[t.career_id] ?? 0), o = this.roller.roll2D(a), c = o.total >= Number(i.target ?? 0);
  ...
  commissionRoll(e) {
    const s = f(e), t = this.requireCurrentTerm(s), r = this.rules.career(t.career_id), n = r.commission;
  ...
  endTerm(e, s = !1, t = "voluntary") {
    const r = f(e), n = this.requireCurrentTerm(r), i = this.rules.career(n.career_id);
  ...
  musterOutRoll(e, s, t = "benefit") {
    var B;
    const r = f(e), n = s ? [...r.completed_careers].reverse().find((E) => E.career_id === s) : r.completed_careers[r.completed_careers.length - 1];
    ...
    const a = n.final_rank >= 5 ? 1 : 0, o = t === "cash" && r.skills.some((E) => E.name.toLowerCase() === "gambler") ? 1 : 0, c = i.mustering_out_dm_characteristic ? A(h(r, i.mustering_out_dm_characteristic)) : 0, u = r.dm_next_benefit + a + o + c, d = i.hiver_career ? this.roller.roll2D(u) : this.roller.rollD(6), _ = Object.keys(i.mustering_out ?? {}).filter((E) => /^\d+$/.test(E)).map(Number), m = Math.min(..._, i.hiver_career ? 2 : 1), p = Math.max(..._, 7), v = Math.max(m, Math.min(p, d.total + (i.hiver_career ? 0 : u))), b = ((B = i.mustering_out) == null ? void 0 : B[String(v)]) ?? {}, y = t === "cash" && r.cash_rolls_used < 3 && b.cash != null ? "cash" : "benefit", C = b[y];
    if (y === "cash") {
      const E = Number(C ?? 0);
      if (E < 0)
    ...
    return r.pending_benefit_rolls -= 1, n.benefit_rolls_used += 1, r.dm_next_benefit = 0, r.pending_benefit_rolls <= 0 && (r.phase = "skill_package"), r.notes.push(`Mustering out ${y}: ${C}.`), { career: i, roll: d, tableRoll: v, column: y, result: C, character: r };
  }
  applyInjury(e, s) {
    var o;
    const t = f(e), r = s ? { dice: [], natural: s, total: s, dm: 0 } : this.roller.rollD(6), i = ((o = this.rules.table("injury").entries) == null ? void 0 : o[String(Math.max(1, Math.min(6, r.total)))]) ?? {}, a = this.injuryPending(i, r.total);
  ...
  resolveInjuryChoice(e, s) {
    ...
    const i = Number(r.damage_to_chosen ?? 0), a = Number(r.auto_reduce_others ?? 0), o = ["STR", "DEX", "END"].filter((m) => m !== s), c = Math.min(h(t, s), i), u = o.map((m) => ({ stat: m, loss: Math.min(h(t, m), a) })).filter((m) => m.loss > 0), d = c + u.reduce((m, p) => m + p.loss, 0), _ = d * 5e3;
  ...
  checkDm(e, s) {
    let t = A(h(e, s == null ? void 0 : s.characteristic));
  ...
  applyStatBlock(e, s) {
    for (const [t, r] of Object.entries(s))
      (N.includes(t) || t === "PSI" || t === "CHA") && (g(e, t, h(e, t) + Number(r)), t === "PSI" && (e.psi = h(e, "PSI")));
  }
  ...
  applyChosenSkills(e, s, t, r, n) {
    ...
    for (const c of a.slice(0, Math.max(0, r))) {
      const u = t.find((p) => p.toLowerCase() === c.toLowerCase()) ?? c, [d, _, m] = w(/\d+$/.test(u.trim()) ? u : `${u} ${n}`);
      o.push(k(e, d, m, _, !0));
    }
    return o;
  }
  ...
  hiverAdvancementRoll(e, s, t) {
    var _, m, p, v;
    const r = this.rules.species(e.species_id) ?? this.rules.species("hiver") ?? {}, n = s.hiver_advancement_table ?? r.hiver_advancement_table ?? {}, i = A(h(e, "SOC")) + e.dm_next_advancement + e.dm_permanent_advancement, a = this.roller.roll2D(i), o = Number(n.senior_min ?? 10), c = Number(n.manipulator_min ?? 15), u = t.rank;
    let d = u;
  ...
  applySkillOrStat(e, s, t) {
    ...
    if (!i) return null;
    const [a, o, c] = w(/\d+$/.test(i) ? i : `${i} ${t}`), u = typeof o == "string" && o.toLowerCase() === "any" ? null : o;
  ...
  resolveCareerChoice(e, s, t) {
    const r = f(e), n = s === "event" ? "pending_career_event_choice" : "pending_career_mishap_choice", i = r[n];
    ...
    if (a === "skill_choice" || a === "free_skill_choice") {
      ...
      const o = Number(i.level ?? 1), [c, u, d] = w(/\d+$/.test(t) ? t : `${t} ${o}`);
      k(r, c, d, u, !0);
...
async function Q(l) {
  const e = await Promise.all(K.map(async (s) => [s, await x(`${l}/tables/${s}.json`)]));
...
function ae(l) {
  ...
  return l.associates.map((s) => {
    ...
    return q(s.description || `Unnamed ${$(t)}`, "associate", {
...
function le(l) {
  return l.equipment.map((e) => q(e.name, "item", {
    tl: 0,
    weight: 0,
    cost: 0,
    notes: e.notes ?? "",
...
function q(l, e, s, t = "systems/mgt2e/icons/items/item.svg") {
  const r = Date.now();
  return {
    name: l,
    type: e,
